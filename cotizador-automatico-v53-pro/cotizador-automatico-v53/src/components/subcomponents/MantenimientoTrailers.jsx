 ...
 * MantenimientoTrailers.jsx - Sub-sección Mantenimiento para Trailers
 * 
 * SOLO para Trailers - Móviles tienen su propio MantenimientoMoviles
 * Trailers NO tienen Service (V5.3 Punto 10.6)
 * Carga desde hoja "Mantenimiento trailers" del archivo soporte único
 * Total = Σ(P × Q / 12) de ítems incluidos (mensualizado)
 */

import React, { useState, useMemo } from 'react';
import { MANTENIMIENTO_TRAILERS_PREDEFINIDOS } from '../core/constants.js';
import { Settings, Plus, Trash2, Edit2, Save, FileSpreadsheet, Info } from 'lucide-react';

/**
 * MantenimientoTrailers
 * 
 * Props:
 * - items: array de ítems de mantenimiento del trailer
...
const MantenimientoTrailers = ({
  ...
  const itemsImp = useMemo(() => {
    if (!datosImportados.mantenimientoTrailers?.length) return [];
    return datosImportados.mantenimientoTrailers.filter(i => {
      if (i.Provincia && provinciaSeleccionada && i.Provincia !== provinciaSeleccionada) return false;
      if (i.TipoTrailer && tipoTrailer && i.TipoTrailer !== tipoTrailer) return false;
      return true;
    });
  }, [datosImportados.mantenimientoTrailers, provinciaSeleccionada, tipoTrailer]);
  ...
  const agregarDesdeSelector = (sel) => {
    if (!sel) return;
    let d = MANTENIMIENTO_TRAILERS_PREDEFINIDOS.find(i => i.descripcion === sel) || itemsImp.find(i => i.Descripcion === sel);
    if (d) {
      onAdd({
        id: Date.now(),
        codigo: d.codigo || d.Codigo || `MNT-T-${Date.now()}`,
    ...
    <div className="p-3 bg-amber-900/20 rounded-lg border border-amber-500/30">
        ...
        {!readOnly && (
          ...
          >
            ...
            <optgroup label="Predefinidos">
              {MANTENIMIENTO_TRAILERS_PREDEFINIDOS.map(i => (
      ...
      <div className="mt-2 p-2 bg-slate-700/30 rounded">
        <p className="text-xs text-slate-400 flex items-center gap-1">
          <Info size={12}/>
          Cálculo: (Precio × Frecuencia anual) / 12 = costo mensual
        </p>
      </div>
    </div>
  );
...
export default MantenimientoTrailers;
